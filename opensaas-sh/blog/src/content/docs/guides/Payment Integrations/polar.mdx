...
You can do that by:
1. Navigating to the `Developers` section under dashboard `Settings > General`.
1. Click on the `New Token` button to create a new token.
2. Give your token a name (e.g., "Open SaaS Development").
3. Select an expiration date. `No expiration` is fine for sandbox mode, but discouraged for live mode.
4. Select the minimal scope necessary:
   - `checkouts:write`
   - `customer_sessions:write`
   - `customers:read`
   - `customers:write`
   - `orders:read`
5. Copy the generated token and paste it in your `.env.server` file:
   ```ini title=".env.server"
...
1. Navigate to `Prodcuts` section.
...
4. Configure any remaining optional fields, e.g. **Media**, **Checkout Fields**, **Metadata**, etc.
5. Click `Create Product`.

After creating each product, you'll need to copy the Product ID from the product page and add it to your `.env.server` file. Tap the `⠇` icon in the top-right corner and select `Copy Product ID`. Open SaaS template features a preset of two different subscriptions ("Hobby" and "Pro") and one one-time pruchase ("Credits 10"):
...
1. Go to `Settings > Webhooks` page.
...
- Configured webhook and set `POLAR_WEBHOOK_SECRET` in `.env.server`.
...
Check the Polar dashboard for webhook event logs:

<Image src={polarWebhookLogs} alt="How to check webhook logs in Polar dashboard" loading="lazy" />

And verify the user's subscription status in your database:
