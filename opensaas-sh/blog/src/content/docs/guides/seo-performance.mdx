...
Wasp gives you the ability to add meta tags to your landing page HTML via the app's `head` property, defined in the `src/client/head.wasp.ts` file:

```ts title="src/client/head.wasp.ts" {4-8}
...
Prerendering fixes both. You can turn it on for any route by adding `prerender: true` to its `route` definition:
