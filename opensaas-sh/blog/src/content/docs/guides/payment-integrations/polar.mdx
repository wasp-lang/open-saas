...
After creating each product, you'll need to copy the `Product ID` from the `Prodcuts > Catalogue` page and add it to your `.env.server` file. Tap the `⠇` icon in the top-right corner and select `Copy Product ID`. 

Open SaaS comes with some example products, two subscriptions and one one-time purchase plan:
...
First make sure you have installed [`ngrok`](https://ngrok.com/docs/getting-started/).
