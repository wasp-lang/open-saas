...
A "email sender" provider is configured via the app's `emailSender` field, defined in the `src/server/emailSender.wasp.ts` file.
...
1. First, set up your app's `emailSender` in the `src/server/emailSender.wasp.ts` file by following [this guide](/guides/email-sending/#integrate-your-email-sender). 
...
```
```ts title="src/server/emailSender.wasp.ts" del={2} ins={3}
export const emailSender: EmailSender = {
  provider: "Dummy",
  provider: "SendGrid",
  defaultFrom: {
