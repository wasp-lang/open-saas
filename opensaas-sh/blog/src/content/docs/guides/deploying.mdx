...
3. Paste the webhook forwarding URL in the `URL` field.
4. Set the `Format` to `"Raw"`.
5. Select at least the following events to be sent:
   - `order.paid`
   - `subscription.updated`
6. Click `Save`.
7. Copy the generated webhook secret (a long, random string starting with `polar_whs_`).
8. Add this signing secret to your server's production environment variables under `POLAR_WEBHOOK_SECRET`.
