...
- `schema.prisma`:
  - The `File` model can be found here. Here you can modify the fields to suit your needs.
- `src/file-upload/file-upload.wasp.ts`:
  - The file upload routes, queries, and actions are declared here in the Wasp Spec.
...
```ts title="src/file-upload/file-upload.wasp.ts"
import { job, type Spec } from "@wasp.sh/spec";
import { cleanUpOrphanedFilesS3Job } from "./workers" with { type: "ref" };
...
export const cleanUpOrphanedFilesS3Job: CleanUpOrphanedFilesS3Job<never, void> = async (
  _rawArgs,
  context
) => {
