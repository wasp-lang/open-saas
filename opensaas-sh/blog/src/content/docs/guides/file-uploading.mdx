...
- `src/file-upload/operations.ts`:
  - The `createFileUploadUrl` action lives here and calls the `getUploadFileSignedURLFromS3` within it using your AWS credentials before passing it to the client. This function stores the files in the S3 bucket within folders named after the user's ID, so that each user's files are stored separately.
...
### Choose an access-control model for downloads

The default bucket configuration is private, and each download URL generated by
`getDownloadFileSignedURLFromS3` expires after one hour. However, the template's
`getDownloadFileSignedURL` query accepts any non-empty `s3Key` without checking
whether the caller is authenticated or owns the corresponding `File` record.
This means anyone who obtains an object's key can request a new download URL.

Object keys are generated as `<userId>/<random UUID>.<extension>`, so they are
difficult to guess. Treat that randomness as a bearer capability, not as proof
of authorization: once a key is shared or leaked, it can be used to request new
presigned URLs for as long as the object and query remain available.

This model can be appropriate for public or intentionally shareable files. For
private user files, add authentication and an authorization lookup before
creating the presigned URL. For example, restrict the query to a `File` record
...
Applications with shared workspaces or roles should replace the ownership check
with their own permission model. In every case, authorize the database record
before signing its stored key instead of trusting a client-supplied key alone.
...
```ts title="src/file-upload/operations.ts"
...
To clean up these orphaned files, you can add a cleanup job (that runs at an interval of your choosing) to:
...
```ts title="src/file-upload/workers.ts"
...
async function fetchAllFileKeysFromS3(): Promise<string[]> {
...
async function findAndDeleteOrphanedFilesInS3(
...
To set up file uploads using Multer, follow the [Wasp integration guide](https://wasp.sh/docs/guides/integrations/file-upload).