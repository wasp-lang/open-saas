...
uploading guide](/0.23/guides/file-uploading/#change-the-cors-settings) for details.
...
In the [Payments Processor integration guide](/0.23/guides/payment-integrations/), you set up your API keys using test keys and test product ids. You'll need to get the live/production versions of those keys. To get these, repeat the instructions in the [Integration Guide](/0.23/guides/payment-integrations/) without being in test mode. Add the new keys to your deployed environment secrets.
...
* \[ ] `GOOGLE_ANALYTICS_PRIVATE_KEY`
  (Make sure you convert the private key within the JSON file to base64 first with `echo -n "PRIVATE_KEY" | base64`. See the [Analytics docs](/0.23/guides/analytics/#google-analytics) for more info)
...
After deploying your server, you need to add the correct redirect URIs to the credential settings. For this, refer to [the guides from the Wasp Docs](https://wasp.sh/docs/0.23/auth/social-auth/overview).
...
Before deploying, make sure your app's meta tags, JSON-LD schema, and `llms.txt` reflect your production URL and content so search engines and AI/LLM tools can index and recommend your app correctly. See the [SEO guide](/0.23/guides/seo-performance/) for details.
...
To learn how, please follow the detailed guide for [deploying to Fly via the Wasp CLI](https://wasp.sh/docs/0.23/deployment/deployment-methods/wasp-deploy/fly) from the Wasp documentation. We suggest you follow this guide carefully to get your app deployed.
...
For setting the remaining server-side environment variables, please refer to the [Deploying with the Wasp CLI Guide](https://wasp.sh/docs/0.23/deployment/deployment-methods/wasp-deploy/fly#environment-variables).
:::

### Deploying Manually

If you prefer to deploy manually, your frontend and backend separately, or just prefer using your favorite provider you can follow [Wasp's Manual Deployment Guide](https://wasp.sh/docs/0.23/deployment/deployment-methods/paas).
