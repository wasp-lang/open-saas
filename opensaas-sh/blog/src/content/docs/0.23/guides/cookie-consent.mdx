...
banner:
  content: >
    ...
    swag! 👕</a>
slug: 0.23/guides/cookie-consent
---
...
The cookie consent modal can be found at `app/src/client/components/cookie-consent/` and contains two main files:
...
What's impotant to note for this template is that we are simply using the `onAccept` callbacks to dynamically add or remove our [Google Analytics](/0.23/guides/analytics/#google-analytics) cookies from the page. In order for it to work correctly with your app, you need to add your [Google Analytics ID](/0.23/guides/analytics/#google-analytics) to your `.env.client` file.
