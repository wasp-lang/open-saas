...
banner:
  content: >
    ...
    swag! 👕</a>
slug: 0.23/general/user-overview
---
...
```prisma title="schema.prisma" ins="User: {}"
model User {
...
We use payment processor to handle all of our subscription payments. The `User` entity has a number of fields that are related to payment processors and their ability to access features behind the paywall:

```prisma title="schema.prisma" {3-7}
model User {
  // ...
  paymentProcessorUserId    String?         @unique
  lemonSqueezyCustomerPortalUrl String?     // You can delete this if you're not using Lemon Squeezy as your payments processor.
...
See the client-side [authorization section](/0.23/guides/authorization/) below for more info on how to handle these statuses within your app.
...
You can add more plans by adding more prodcuts to your payment processor and updating environment variables in your `.env.server` file as well as the relevant code in your app.

See the [Payment Integrations Guide](/0.23/guides/payment-integrations/) of a specific payment processor for more info on how to do this.

### User Roles

At the moment, we have two user roles: `admin` and `user`. This is defined within the `isAdmin` field in the `User` entity:

```prisma title="schema.prisma" {5}
model User {
  // ...
  email                     String?         @unique
  username                  String?         @unique
...
If you'd like to give yourself and/or certain users admin privileges, follow the instructions in the [Admin Dashboard](/0.23/general/admin-dashboard/#permissions) section.
:::

As a `user`, a user has access to the user-facing app that sits behind the login, but not the Admin dashboard. You can further restrict access to certain features within the app by following the [authorization guide](/0.23/guides/authorization/).
