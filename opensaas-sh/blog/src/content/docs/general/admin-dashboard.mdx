...
These metrics are aggregated within the background job `calculateDailyStatsJob`, which by default is run every hour. You can change the frequency of this job by modifying its `cron` field:

```ts title="src/analytics/analytics.wasp.ts" {8}
